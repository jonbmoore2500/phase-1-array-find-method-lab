...
const record = [
  ...
  // inefficient, must be a better way
...
// }
  ...
  // same ineffeciency, but takes up slightly less space
  //since it doesn't repeat the .find() each time
  function superbowlWin(array) {
   ...
   // returns undefined if there is no "W" but 
   // returns a whole object instead of just a year
// function superbowlWin(array) {
//     return array.find(item => item.result === "W");
// }

   // returns a year if there is a "W" but can't run .year on undefined so 
   // it breaks if there isn't a "W"
// function superbowlWin(array) {
//     return array.find(item => item.result === "W").year;
// }
